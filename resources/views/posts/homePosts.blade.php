...
@section('header-intro')
    ...
    @if(request()->input('busca'))
        <h2 style="text-align: center;">@section('title', 'Busca por: '.request()->input('busca') . ' (' .$posts->total() . ')')</h2>
    @endif

    <h2 style="text-align: center;">@section('title', 'Últimos Posts')</h2>
...
@section('conteudo')
    ...
    <div class="col-lg-12">
        ...
        <div class="row">
            ...
            @empty
                <h4>
                    <center>Nenhum post encontrado.</center>
                </h4>
            @endforelse
        </div>
        @if(request()->input('busca'))
            {{ $posts->appends(['busca' => request()->input('busca')])->links() }}
        @endif
    </div>
